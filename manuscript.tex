...
Ensemble weighting methods based on the Shapley value entangle each model's standalone quality with its interaction effects, and attempts to correct this by augmenting Shapley values with the Shapley Interaction Index introduce systematic double-counting of pairwise effects already embedded within the Shapley value. This study proposes a coopetitive ensembling framework that resolves both problems by applying the M\"{o}bius decomposition (Harsanyi dividends) to construct ensemble weights from mathematically non-overlapping singleton and pairwise terms. Singleton dividends capture each model's pure standalone contribution; pairwise dividends capture the surplus or deficit from model interactions exclusively. A tunable parameter $\lambda$ balances individual model quality against interaction structure, and softmax normalization converts the resulting scores to valid probability weights.

The framework was evaluated on fifteen binary classification benchmark datasets using a pool of five heterogeneous base models (Logistic Regression, Random Forest, SVM, XGBoost, and $k$-Nearest Neighbors) with five random seeds per dataset. The coopetitive variant ($\lambda = 0.5$) achieved the highest mean AUC-ROC of any ablation variant tested (0.8977) and the best probability calibration (negative Brier score $= -0.097$). The Shapley+SII variant, which introduces double-counting, degraded performance by 0.009 AUC relative to plain Shapley, providing evidence consistent with the double-counting hypothesis. The primary hypothesis that adding interaction information significantly improves over performance-weighted weighting (H1) was not statistically supported ($p = 0.148$); the coopetitive framework ranked second of six methods in the benchmark comparison ($p = 0.072$ by Friedman test). The most durable practical contribution is the pairwise dividend matrix, which provides an interpretable diagnostic of model complementarity and redundancy that no other ensemble weighting method produces.
...
This study demonstrates that the M\"{o}bius decomposition can serve as a principled diagnostic instrument for ensemble design, providing interaction-level transparency that simpler combination methods do not offer. The pairwise dividend matrix and the coopetitive framework built around it extend the applicability of cooperative game theory within machine learning, establishing a formal connection between ensemble weighting, Harsanyi dividends, and interpretable model diagnostics. This contribution is relevant beyond the specific experimental setting: any practitioner who needs to understand why an ensemble weights its members as it does, or who wants to identify which models contribute through complementarity versus which overlap through redundancy, can apply the dividend matrix directly without committing to the full coopetitive weighting formula. For the field of ensemble learning, the framework's most durable value is this diagnostic transparency---a quality that performance-focused methods like stacking, no matter how accurate, do not share.
